...
// Current instance being ticked. The two CPUs are stepped from one machine loop,
...
#define bus_write(a, v) (C->wr(C->ctx, (uint16_t)(a), (uint8_t)(v)))
    ...
    switch (cpu.cycle) {
        ...
        case 4:
            ...
            bus_write(STACK(cpu.sp),
                      (uint8_t)((cpu.p & 0xCF) | 0x20 | (cpu.int_b ? 0x10 : 0)));
            cpu.sp--;
            cpu.cycle = 5;
            break;
        case 5:
            cpu.data = bus_read(cpu.int_vector);  // vector low
            cpu.p |= FLAG_I;                  // set I after P was pushed
            cpu.cycle = 6;
            break;
        case 6:
            cpu.pc = (uint16_t)((bus_read((uint16_t)(cpu.int_vector + 1)) << 8) |
...
static void begin_interrupt(bool nmi) {
    ...
    cpu.int_b = false;  // hardware interrupts push B clear
    cpu.int_vector = nmi ? 0xFFFA : 0xFFFE;
    if (nmi) {
...
// NMI edge detection, run every cycle. cpu.nmi_line is active-low; a high-to-low
// transition latches a pending NMI that is serviced exactly once.
static void poll_nmi_edge(void) {
    if (cpu.nmi_last == 1 && cpu.nmi_line == 0) {
        cpu.nmi_pending = 1;
    }
...
void cpu6502_reset(CPU6502 *c) {
    // PC loaded from the reset vector at $FFFC/$FFFD; the owner sets up any
    // banking (e.g. the C64's 6510 port) before calling so the vector reads
    // from the right memory.
    // invariant: simplified reset (direct vector load); the full cycle-exact
    // 7-cycle reset sequence is not required by the suite.
    C = c;
    cpu.sp = 0xFD;
    cpu.p |= FLAG_I;
...
uint8_t cpu6502_halt_opcode(const CPU6502 *c) { return c->halt_opcode; }

bool cpu6502_jammed(const CPU6502 *c) { return c->jammed; }
