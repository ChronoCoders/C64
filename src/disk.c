...
bool disk_writeback(void) {
    ...
    // Write a sibling temp file and replace the original by an atomic same-directory
    // rename only after every write, the flush to stable storage, and the close have
    // succeeded. Any failure removes the temp and leaves the original untouched.
    size_t plen = strlen(mount_path);
    char tmp_path[sizeof(mount_path) + 8];
    if (plen + sizeof(".tmp") > sizeof(tmp_path)) { return false; }  // no truncation
    memcpy(tmp_path, mount_path, plen);
    memcpy(tmp_path + plen, ".tmp", sizeof(".tmp"));

    FILE *f = fopen(tmp_path, "wb");
    if (f == NULL) { return false; }
    size_t want = has_error_info ? D64_ERR_SIZE : D64_STD_SIZE;
